...
const ModeCard = ({ title, imageUrl, isSelected, onClick, selectedColor }: ModeCardProps) => {
    return (
        <vstack
            width={"80px"}
            height={"100px"}
            ...
            backgroundColor={isSelected ? "#19272C" : "transparent"}
            onPress={onClick}
        >
            <image imageHeight={"55px"} imageWidth={"55px"} url={imageUrl} />
            <text size="large" weight="bold" color={isSelected ? selectedColor : "white"}>
                {title}
            </text>
        </vstack>
    );
